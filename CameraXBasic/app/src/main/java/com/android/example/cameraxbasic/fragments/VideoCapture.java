...
public final class VideoCapture extends UseCase {
    ...
    private final AtomicBoolean mEndOfLifeSignal = new AtomicBoolean(false);
    private final AtomicBoolean mNotifyResult = new AtomicBoolean(false);
    private CountDownLatch mRecordingFinished = null;
    ...
    public VideoCaptureConfig getVideoCaptureConfig() {
        return (VideoCaptureConfig) getCurrentConfig();
    }
    ...
    protected Size onSuggestedResolutionUpdated(@NonNull Size suggestedResolution) {
        ...
        try {
            try {
                VideoCaptureConfig config = getVideoCaptureConfig();
                MediaFormat format = createMediaFormat(config, suggestedResolution);
                String encoder = new MediaCodecList(MediaCodecList.REGULAR_CODECS).findEncoderForFormat(format);
                if (encoder != null) {
                    mVideoEncoder = MediaCodec.createByCodecName(encoder);
                } else {
    ...
    /**
     * Starts recording video, which continues until {@link VideoCapture#stopRecording(boolean)} ()} is
     * called.
     *
    ...
    public void startRecording(
        ...
        OnVideoSavedCallback postListener = new VideoSavedListenerWrapper(executor, callback, mNotifyResult);
        ...
        try {
            ...
            postListener.onError(ERROR_ENCODER, "Video encoder start fail", e);
            return;
        }
        ...
        mRecordingFinished = new CountDownLatch(1);
        mIsRecording = true;
        ...
        if (!errorOccurred) {
            long duration = mEndTimestamp - mStartTimestamp;
            postListener.onVideoSaved(new OutputFileResults(mSavedVideoUri, duration));
            mSavedVideoUri = null;
        }
        recordingCompleter.set(null);
    });
     ...
     */
    public void stopRecording(boolean notifyResult) {
        if (Looper.getMainLooper() != Looper.myLooper()) {
            CameraXExecutors.mainThreadExecutor().execute(() -> stopRecording(notifyResult));
            return;
        }
        Logger.i(TAG, "stopRecording");
        notifyInactive();
        if (!mEndOfAudioVideoSignal.get() && mIsRecording) {
            mNotifyResult.set(notifyResult);
    ...
    private void releaseResources() {
        mEndOfLifeSignal.set(true);
        mVideoHandlerThread.quitSafely();

        if (mCameraSurface != null) {
            try {
                CountDownLatch finished = mRecordingFinished;
                if (finished != null) {
    ...
    private boolean writeVideoEncodedBuffer(int bufferIndex) {
        ...
        if (mVideoTrackIndex >= 0 && mVideoBufferInfo.size > 0) {
            ...
            synchronized (mMuxerLock) {
                mEndTimestamp = TimeUnit.MICROSECONDS.toMillis(mVideoBufferInfo.presentationTimeUs);
    ...
    @NonNull Size resolution) {
        ...
        try {
            while (!videoEos && !errorOccurred && !mEndOfLifeSignal.get()) {
                // Check for end of stream from main thread
                if (mEndOfVideoStreamSignal.getAndSet(false)) {
    ...
    private static final class VideoSavedListenerWrapper implements OnVideoSavedCallback {
        ...
        public void onVideoSaved(@NonNull OutputFileResults outputFileResults) {
            ...
            try {
                mExecutor.execute(() -> {
                    if (mEnabled.get()) {
                        mOnVideoSavedCallback.onVideoSaved(outputFileResults);
                    }
                });
    ...
    public static class OutputFileResults {
        ...
        OutputFileResults(@Nullable Uri savedUri, long durationMs) {
            mSavedUri = savedUri;
